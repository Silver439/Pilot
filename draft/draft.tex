...
The algorithm's core mechanism involves using an optimistic estimate of the mean scores to guide exploration. Specifically, at each iteration $n$, we construct the lower confidence bound (LCB) vector $\boldsymbol{\mu}_n - \beta_n \boldsymbol{\sigma}_n$, where $\boldsymbol{\sigma}_n$ contains the posterior standard deviations. This LCB vector represents an optimistic estimate of the true means from the perspective of minimizing the expected minimum score. We then pass this optimistic estimate to a heuristic oracle from Section 4.3, which returns a module set $S_n$ for evaluation. The observed scores from testing $S_n$ are used to update our posterior belief about $\boldsymbol{\theta}$. The complete procedure is detailed in Algorithm \ref{alg:gp_ucb_algorithm_adaptive}.

\begin{algorithm}
\caption{GP-UCB Algorithm with Adaptive Exploration}
...
\For{$i = 1$ to $N_0$} \Comment{Initial phase: ensure each module observed at least once}
...
\EndFor
\For{$n = N_0+1$ to $N$} \Comment{Main optimization phase}
    \State $\boldsymbol{\sigma}_{n-1} \gets \sqrt{\text{diag}(\boldsymbol{\Sigma}_{n-1})}$ \Comment{Compute posterior standard deviations}
    \State $\boldsymbol{\beta}_n[x] \gets \sqrt{6 \ln n / \mathbf{T}_{n-1}[x]}$ for each $x \in \mathcal{X}$ \Comment $\mathbf{T}_{n-1}[x]$ represents the number of times module $x$ was observed before round $n-1$
    \State $\boldsymbol{\tilde{\mu}}_n \gets \boldsymbol{\mu}_{n-1} - \boldsymbol{\beta}_n \circ \boldsymbol{\sigma}_{n-1}$ \Comment{Construct LCB-based mean estimate}
    \State $S_n \gets \mathcal{O}(\boldsymbol{\tilde{\mu}}, \boldsymbol{\Lambda})$ \Comment{Query selection oracle}
    \State Observe score vector $\mathbf{y}_n$ for modules $S_n$ on a new pilot.
    \State Update $(\boldsymbol{\mu}_n, \boldsymbol{\Sigma}_n)$ with $(\mathbf{y}_n, S_n)$; Update $\mathbf{T}_n[x] \gets \mathbf{T}_n[x] + 1$ for each $x \in S_n$
    \State Calculate $R(S_n) = \mathbb{E}[\min\,\{Y(x) : x \in S_n\}]$, track minimal $R(S_n)$ and corresponding $S_n$ 
\EndFor
